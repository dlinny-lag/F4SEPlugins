Scriptname DS:{keyTypeName}Dict{valueTypeName}Array Native Hidden Const

; creates an <{keyTypeName}, {valueTypeName}Array> dictionary
...
; Deletes previously created <{keyTypeName}, {valueTypeName}Array> dictionary
; returns true on success
...
int function ArrayLength(Keyword identifier, {keyType} key) global native
...
; - no dictionary found by identifier
; - no array found by specified key
Result function GetElement(Keyword identifier, {keyType} key, int index) global native
...
; - identifier is invalid 
; - no array found by specified key
bool function AddElement(Keyword identifier, {keyType} key, {valueType} value) global native
...
; - no dictionary found by identifier
; - no array found by specified key
bool function AddRange(Keyword identifier, {keyType} key, {valueType}[] toAdd) global native
...
; - no dictionary found by identifier
; - no array found by specified key
bool function SetElement(Keyword identifier, {keyType} key, {valueType} value, int index) global native
...
; - no dictionary found by identifier
; - no array found by specified key
bool function RemoveAtIndex(Keyword identifier, {keyType} key, int index) global native
...
bool function InsertElement(Keyword identifier, {keyType} key, {valueType} value, int index) global native
...
; - no element found
int function IndexOf(Keyword identifier, {keyType} key, {valueType} value, int startIndex) global native
...
; - identifier is invalid 
; - no array found by specified key
bool function SortArray(Keyword identifier, {keyType} key, int mode) global native
...
; appends elements of DS:{keyTypeName}Array specified by arrayId to array specified by key
; if array is not exist for this key then new array is created
; returns true if element was added successfully
; returns false if
; - identifier is invalid 
...
; - arrayId is invalid
; - no array with arrayId was found
bool function AppendArray(Keyword identifier, {keyType} key, Keyword arrayId) global native

; copies elements of array specified by key to DS:{keyTypeName}Array specified by destinationArrayId
; returns true on success
; returns false if
; - identifier is invalid 
; - no dictionary found by specified identifier 
...
bool function CopyToArray(Keyword identifier, {keyType} key, Keyword targetArrayId) global native
