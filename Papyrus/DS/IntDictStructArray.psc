Scriptname DS:IntDictStructArray Native Hidden Const

; creates <Int, StructArray> dictionary
...
; Deletes previously created <Int, StructArray> dictionary
; returns true on success
...
int function ArrayLength(Keyword identifier, int key) global native
...
var function GetElement(Keyword identifier, int key, int index) global native
...
; returns false if
; - identifier is invalid 
bool function AddElement(Keyword identifier, int key, var value) global native
...
; - identifier is invalid 
; - no dictionary found by identifier
bool function AddRange(Keyword identifier, int key, var[] toAdd) global native
...
; - no dictionary found by identifier
; - no array found by specified key. one could use Contains() to detect if array present in dictionary
bool function SetElement(Keyword identifier, int key, var value, int index) global native
...
; - no dictionary found by identifier
; - no array found by specified key. one could use Contains() to detect if array present in dictionary
bool function RemoveAtIndex(Keyword identifier, int key, int index) global native
...
; - no array found by specified key. one could use Contains() to detect if array present in dictionary
; - index is out of range
bool function InsertElement(Keyword identifier, int key, var value, int index) global native
...
int function IndexOf(Keyword identifier, int key, string fieldName, var fieldValue, int startIndex) global native
...
; - no dictionary found by identifier
; - no array found by specified key. one could use Contains() to detect if array present in dictionary
bool function SortArray(Keyword identifier, int key, string fieldName, int mode) global native
...
; appends elements of DS:IntArray specified by arrayId to array specified by key
; if array is not exist for this key then new array is created
; returns true if element was added successfully
; returns false if
; - identifier is invalid 
...
bool function AppendArray(Keyword identifier, int key, Keyword arrayId) global native

; copies elements of array specified by key to DS:IntArray specified by destinationArrayId
; returns true on success
; returns false if
; - identifier is invalid 
; - no dictionary found by specified identifier 
...
bool function CopyToArray(Keyword identifier, int key, Keyword targetArrayId) global native
